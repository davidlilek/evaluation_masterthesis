...
path = "N:/1_A_Bachelor_Master_Intern/00_M_2022/David/Data/3_20220406_TR/20220406_TR_QC_noMBR_rerun_4masterthesis/"
...
i = 0
res_pool_razor <- c()
for (i in 1:length(sample_names_raw)){
  tmp <- apply(results_razor[[i]], 1, any)
  res_pool_razor <- c(res_pool_razor, length(tmp[tmp == TRUE]))
}
```
...
res_pool_unique <- c()
for (i in 1:length(sample_names_raw)){
  tmp <- apply(results_unique[[i]], 1, any)
  res_pool_unique <- c(res_pool_unique, length(tmp[tmp == TRUE]))
}
```
...
for (i in 1:length(sample_names_raw)){
    tmp <- results_lfq[[i]][rowSums(is.na(results_lfq[[i]])) != ncol(results_lfq[[i]]), ]
  res_pool_lfq <- c(res_pool_lfq, nrow(tmp))
}
```
...
```{r}
res <- data.frame(cbind(res_pool_razor,res_pool_unique,res_pool_lfq))
colnames(res) <- c("Razor", "Unique", "LFQ")
write.csv2(res, file = "./tmpfiles/20220406_TR_QC_noMBR_rerun.csv")
```
