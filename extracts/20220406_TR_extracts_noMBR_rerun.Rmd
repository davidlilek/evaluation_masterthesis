...
path = "N:/1_A_Bachelor_Master_Intern/00_M_2022/David/Data/3_20220406_TR/20220406_TR_extracts_noMBR_rerun_4masterthesis/"
...
for (i in 1:10){
  ...
  res_pool_razor_A <- c(res_pool_razor_A, length(tmp[tmp == TRUE]))
  tmp <- apply(results_razor[[i]][4:6], 1, any)
  res_pool_razor_B <- c(res_pool_razor_B, length(tmp[tmp == TRUE]))
  tmp <- apply(results_razor[[i]][7:9], 1, any)
  res_pool_razor_C <- c(res_pool_razor_C, length(tmp[tmp == TRUE]))
  tmp <- apply(results_razor[[i]][10:12], 1, any)
  res_pool_razor_D <- c(res_pool_razor_D, length(tmp[tmp == TRUE]))
  tmp <- apply(results_razor[[i]][13:15], 1, any)
  res_pool_razor_E <- c(res_pool_razor_E, length(tmp[tmp == TRUE]))
  tmp <- apply(results_razor[[i]][16:18], 1, any)
  res_pool_razor_F <- c(res_pool_razor_F, length(tmp[tmp == TRUE]))
}
```
...
for (i in 1:10){
  ...
  res_pool_unique_A <- c(res_pool_unique_A, length(tmp[tmp == TRUE]))
  tmp <- apply(results_unique[[i]][4:6], 1, any)
  res_pool_unique_B <- c(res_pool_unique_B, length(tmp[tmp == TRUE]))
  tmp <- apply(results_unique[[i]][7:9], 1, any)
  res_pool_unique_C <- c(res_pool_unique_C, length(tmp[tmp == TRUE]))
  tmp <- apply(results_unique[[i]][10:12], 1, any)
  res_pool_unique_D <- c(res_pool_unique_D, length(tmp[tmp == TRUE]))
  tmp <- apply(results_unique[[i]][13:15], 1, any)
  res_pool_unique_E <- c(res_pool_unique_E, length(tmp[tmp == TRUE]))
  tmp <- apply(results_unique[[i]][16:18], 1, any)
  res_pool_unique_F <- c(res_pool_unique_F, length(tmp[tmp == TRUE]))
}
```
...
for (i in 1:10){
  ...
  res_pool_lfq_A <- c(res_pool_lfq_A, nrow(tmp))
  
  sub <- results_lfq[[i]][4:6]
  tmp <- sub[rowSums(is.na(sub)) != ncol(sub), ]
  res_pool_lfq_B <- c(res_pool_lfq_B, nrow(tmp))
  
  sub <- results_lfq[[i]][7:9]
  tmp <- sub[rowSums(is.na(sub)) != ncol(sub), ]
  res_pool_lfq_C <- c(res_pool_lfq_C, nrow(tmp))
  
  sub <- results_lfq[[i]][10:12]
  tmp <- sub[rowSums(is.na(sub)) != ncol(sub), ]
  res_pool_lfq_D <- c(res_pool_lfq_D, nrow(tmp))
  
  sub <- results_lfq[[i]][13:15]
  tmp <- sub[rowSums(is.na(sub)) != ncol(sub), ]
  res_pool_lfq_E <- c(res_pool_lfq_E, nrow(tmp))
  
  sub <- results_lfq[[i]][16:18]
  tmp <- sub[rowSums(is.na(sub)) != ncol(sub), ]
  res_pool_lfq_F <- c(res_pool_lfq_F, nrow(tmp))
  }
```
...
```{r}
res <- data.frame(cbind(res_pool_razor_A,
                        res_pool_razor_B,
                        res_pool_razor_C,
                        res_pool_razor_D,
                        res_pool_razor_E,
                        ...
                        res_pool_lfq_E,
                        res_pool_lfq_F))
colnames(res) <- paste(rep(c("Razor_", "Unique_", "LFQ_"),each = 6),rep(LETTERS[1:6],1),sep="")
write.csv2(res, file = "./tmpfiles/20220406_TR_extracts_noMBR_rerun.csv")
```
