...
for (sample_name in sample_names){
  ...
  p_plot_two <- ggplot(df, aes(x=Type, y=no, fill=Evaluation)) +
    ...
    plots_no_twopeptides[[sample_name]] <- p_plot_two
    plots_rsd_twopeptides[[sample_name]] <- rsd_plot_two
    results_two[[sample_name]] <- df
}
```

```{r}
saveRDS(plots_no_twopeptides, file = "plots_no_secondpeptides_twopeptides.rds")
saveRDS(plots_rsd_twopeptides, file = "plots_rsd_secondpeptides_twopeptides.rds")
saveRDS(results_two, file = "results_seconpeptides_twopeptides.rds")
```

```{r eval=FALSE, include=FALSE}
df_onepeptide <- readRDS("20220406_TR_QC_summary_rerun_secondpeptides_onepeptide.rds")
