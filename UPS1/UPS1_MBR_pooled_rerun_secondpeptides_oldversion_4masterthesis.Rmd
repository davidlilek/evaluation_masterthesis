...
unique <- 1  #how many peptides should be necessary to identify a protein

RAZOR <- TRUE #TRUE | FALSE
UNIQUE <- TRUE #TRUE | FALSE
LFQ <- TRUE #TRUE | FALSE

path = "N:/1_A_Bachelor_Master_Intern/00_M_2022/David/Data/2_UPS1/UPS1_MBR_pooled_secondpeptides_rerun_4masterthesis/"
...
if (LFQ == TRUE) {
  ...
  for (x in file.list){
   ...
   results_lfq_human[[x]] <- dplyr::filter(results,grepl("Human",results$FASTA))[-6]
...
# save lfq human
saveRDS(results_lfq_human, "./tmpfiles/lfq_human_MBR_pooled_secondpeptides.rds")
...
write.csv2(res, file = "./tmpfiles/UPS1_MBR_pooled_rerun_secondpeptides.csv")
```

```{r}
res <- data.frame(cbind(results_2gether_razor_human,results_2gether_unique_human,results_2gether_lfq_human))
colnames(res) <- paste(rep(c("Razor_", "Unique_", "LFQ_"),each = 5),rep(colnames(results_2gether_lfq)[-6],3),sep="")
write.csv2(res, file = "./tmpfiles/UPS1_MBR_pooled_rerun_secondpeptides_human.csv")
```
