...
path = "N:/1_A_Bachelor_Master_Intern/00_M_2022/David/Data/evaluation_masterthesis/UPS1/tmpfiles/UPS1_oneuniquepeptide/"

##########read in all data with pattern .txt
# get file list
file.list <- list.files(path = path, pattern='*.csv')
# remove manually pooling and oldversion
...
for (sample_name in sample_names){
  ...
  p_plot_one <- ggplot(df, aes(x=Type, y=no, fill=Evaluation)) +
    ...
    plots_no_onepeptide[[sample_name]] <- p_plot_one
    plots_rsd_onepeptide[[sample_name]] <- rsd_plot_one
    res_df[[sample_name]] <- df
}
```

```{r}
saveRDS(plots_no_onepeptide, file = "plots_no_secondpeptides_onepeptide.rds")
saveRDS(plots_rsd_onepeptide, file = "plots_rsd_secondpeptides_onepeptide.rds")
saveRDS(res_df, file = "results_secondpeptides_onepeptide.rds")
```
# LaTeX
